...
3. Navigate to `../apache/conf/extra` and edit the `httpd-vhosts.conf` file with a text editor, and add the code snippet below with your own `project_name` and path:
    ```markup
    <VirtualHost *:80>
        # highlight-start
...
5. Click on `Add Redirect` then set it to `{APP_URL}/discord/callback`, e.g. https://project.manager-test/osu/callback.
    :::info
